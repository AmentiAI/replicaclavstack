...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    // Outer wrapper is a Link — entire card navigates to product page
    <Link
      href={detailLink}
      className="group relative bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col hover:border-amber-300 hover:shadow-lg hover:shadow-amber-100/50 transition-all duration-300"
    >
      {/* Image area */}
      <div className="relative aspect-square bg-gray-50 overflow-hidden">
        ...
        {badge && (
          <div
            className="absolute top-2 left-2 sm:top-3 sm:left-3 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-bold border"
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-5">
        ...
        <div className="flex gap-1 sm:gap-1.5 mb-3 sm:mb-4 flex-wrap">
          <span className="px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] bg-gray-100 text-gray-600 border border-gray-200">
            &gt;98% HPLC
          </span>
          <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] border ${
            product.in_stock
        ...
        <div className="flex gap-2">
          <span className="flex-1 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium text-center text-gray-600 bg-gray-50 border border-gray-200 group-hover:border-amber-200 group-hover:text-amber-700 transition-all">
            Details
          </span>
          <a
            href={shopLink}
            ...
            className="flex-1 btn-primary py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold text-center inline-flex items-center justify-center gap-1"
          >
            Shop
